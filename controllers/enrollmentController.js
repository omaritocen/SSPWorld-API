...
module.exports.postEnrollment = async (req, res) => {
    ...
    if (isEnrolled)
        return res.status(400).send({ error: 'User is already enrolled in this course' });
...
module.exports.deleteEnrollment = async (req, res) => {
    ...
    if (!enrollment)
        return res
            .status(404)
            .send({ error: 'No enrollment with this id is found' });
...
module.exports.getEnrollmentsByCourseId = async (req, res) => {
    ...
    if (!enrollment)
        return res.status(404).send({
            error: 'No enrollment for this student with this course id',
        });
